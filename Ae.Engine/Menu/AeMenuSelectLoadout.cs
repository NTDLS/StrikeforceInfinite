...
namespace Ae.Engine.Menu
{
    ...
    /// </summary>
    internal class AeMenuSelectLoadout
        : AeMenu
    {
        class SelectedSprite
        {
            ...
            public SelectedSprite(string assetKey, AeSpritePlayer sprite)
            {
                Sprite = sprite;
                AssetKey = assetKey;
            }
        ...
        private readonly AeSpriteMenuItem _shipBlurb;
            ...
            : base(engine)
        {
            ...
            List<AeSpritePlayer> playerSprites = new();

            float previousSpriteSize = 0;

            foreach (var playerShipContainer in playerShipContainers)
            {
                var playerSprite = engine.Sprites.Add<AeSpritePlayer>(playerShipContainer.Key);
        ...
        private void PlayerLoadoutMenu_OnSelectionChanged(AeSpriteMenuItem item)
        {
            if (item.UserData is SelectedSprite selectedSprite)
            {
